      ...
      >
        ...
        <form onSubmit={handleSubmit} className="space-y-6 relative z-10">
          ...
          <div>
            ...
            <div className="grid grid-cols-2 gap-3">
              <label className={`flex flex-col items-center justify-center gap-1 cursor-pointer p-3 rounded-xl border transition-all ${role === 'PATIENT' ? 'bg-cyan-500/20 border-cyan-500/50 text-cyan-300 shadow-[0_0_15px_rgba(6,182,212,0.2)]' : 'bg-white/5 border-white/10 text-gray-400 hover:bg-white/10'}`}>
                <input
                  ...
                  className="hidden"
                />
                <span className="font-bold text-xs uppercase tracking-wider">Patient</span>
              </label>
              <label className={`flex flex-col items-center justify-center gap-1 cursor-pointer p-3 rounded-xl border transition-all ${role === 'DOCTOR' ? 'bg-cyan-500/20 border-cyan-500/50 text-cyan-300 shadow-[0_0_15px_rgba(6,182,212,0.2)]' : 'bg-white/5 border-white/10 text-gray-400 hover:bg-white/10'}`}>
                <input
                  type="radio"
                  name="role"
                  checked={role === 'DOCTOR'}
            ...
            {error.role && (
              <p className="text-red-400 text-sm mt-2 font-medium">{error.role[0]}</p>
            )}
          </div>
